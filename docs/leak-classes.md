...
**Why it leaks**: late-night operator work makes timestamps load-bearing for distinguishing decisions. Day-relative words reveal the writer's frame of reference is the operator's calendar, not the project's history.
...
**Why it leaks**: ADR prose treats the operator's interventions as load-bearing decision context. They are, but in operator-private docs.
